...
*** trait 一致性
    ...
    除了在本地定义 ~Add~ trait方法, 还可以在本地创建一个新的类型, 然后为此新类型
    实现 ~Add~, 这同样不会违反孤儿原则.

    注意: *关联类型 Output 必须指定具体类型*.

*** trait 继承
    Rust 不支持传统面向的集成, 但是 *支持 trait继承*. 子 trait 可以继承父 trait
...
**** trait 对象
     ...
     型, 这就是 trait *对象(trait Object)*. "对象" 这个词来自于卖弄项对象编程语
     ...
     *trait 对象* 增加了新的语法 *dyn Trait*, 其中 dync 是 Dynamic(动态) 的缩写.
...
**** Send trait 和 Sync trait
     ...
     是标记, 可以安全地跨线程传递和访问的类型用 Send 和 Sync标记, 否则用 ~!Send~
     和 ~!Sync~ 标记.
...
** 类型转换
   ...
   换, 如果不多加注意, 可能会得到意料之外的结果. eg2: C 语言不通大小类型相互转换,
